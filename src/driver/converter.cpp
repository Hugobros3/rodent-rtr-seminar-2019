...
void build_bvh<8, 4>(const obj::TriMesh &tri_mesh,
    ...
    std::cout << "nodes_count:" << bvh8.nodes_count << " primitives_count: " << bvh8.tris_count << "\n";

    for(int i = 0; i < bvh8.nodes_count; i++)
...
static size_t cleanup_obj(obj::File &obj_file, obj::MaterialLib &mtl_lib) {
    ...
    if (used_mtls.size() != obj_file.materials.size()) {
        ...
        for (auto mtl_name : obj_file.materials) {
            ...
            auto new_mtl_index =
                    std::find(new_materials.begin(), new_materials.end(), mtl_name) - new_materials.begin();
        ...
        info("The scene has ", num_complex, " complex material(s), and ", new_materials.size() - num_complex,
             " simple material(s)");
    }
    return num_complex;
}
...
static bool
convert_obj(const std::string &file_name, Target target, size_t dev, size_t max_path_len, size_t spp, bool embree_bvh,
            bool fusion, std::ostream &os) {
    ...
    for (size_t i = 0; i < tri_mesh.indices.size(); i += 4) {
        ...
        if (has_map_ke) {
            ...
            if (mat.map_ke != "") {
                os << "        make_texture(math, make_repeat_border(), make_bilinear_filter(), image_"
                   << make_id(image_names[images[mat.map_ke]]) << ")\n";
            } else {
    ...
    for (auto &mtl_name : obj_file.materials) {
        ...
        if (mat.illum == 5) {
            os << "        let bsdf = make_mirror_bsdf(math, surf, make_color(" << mat.ks.x << "f, " << mat.ks.y
               << "f, " << mat.ks.z << "f));\n";
        } else if (mat.illum == 7) {
            os << "        let bsdf = make_glass_bsdf(math, surf, 1.0f, " << mat.ni << "f, " << "make_color("
               << mat.ks.x << "f, " << mat.ks.y << "f, " << mat.ks.z << "f), make_color(" << mat.tf.x << "f, "
               << mat.tf.y << "f, " << mat.tf.z << "f));\n";
        } else {
            ...
            if (has_diffuse) {
                if (mat.map_kd != "") {
                    os
                            << "        let diffuse_texture = make_texture(math, make_repeat_border(), make_bilinear_filter(), image_"
                            << make_id(image_names[images[mat.map_kd]]) << ");\n";
                    os << "        let kd = diffuse_texture(vec4_to_2(surf.attr(0)));\n";
                } else {
                    os << "        let kd = make_color(" << mat.kd.x << "f, " << mat.kd.y << "f, " << mat.kd.z
                       << "f);\n";
                }
            ...
            if (has_specular) {
                if (mat.map_ks != "") {
                    os
                            << "        let specular_texture = make_texture(math, make_repeat_border(), make_bilinear_filter(), image_"
                            << make_id(image_names[images[mat.map_ks]]) << ");\n";
                    os << "        let ks = specular_texture(vec4_to_2(surf.attr(0)));\n";
                } else {
                    os << "        let ks = make_color(" << mat.ks.x << "f, " << mat.ks.y << "f, " << mat.ks.z
                       << "f);\n";
                }
                os << "        let ns = " << mat.ns << "f;\n";
